...
class GoodGameChat extends EventEmitter {
    ...
    authorize(login, password) {
        ...
        return self._getToken(login, password).then(function(userInfo) {
            ...
            return self.waitForEvent(API.ACCEPT_CREDS).then(function(userInfo) {
                return new Client(userInfo.user_id, userInfo.user_name, userInfo.isMobile);
            });
        });
    }
    ...
    getChannels(start, count) {
        ...
        return self.waitForEvent(API.CHANNELS).then(function(channelsJSON) {
    ...
    joinChannel(channel, isHidden) {
        ...
        return self.waitForEvent(API.JOINED_TO_CHANNEL).then(function(channelInfo) {
    ...
    getUsersInChannel(id) {
        ...
        self._send(API.GET_USERS, {
            channel_id: id 
        });
        
        return self.waitForEvent(API.USERS).then(function(usersInfo) {
